...
*/
export default function measureDistance(lat1: number, long1: number,
    lat2: number, long2: number): number {
    ...
    return Math.round(
        (2 * r * Math.asin(Math.sqrt(
            (1/2) * (1 - Math.cos(lat2Rad - lat1Rad) + (
                Math.cos(lat1Rad) * Math.cos(lat2Rad) * (1 - Math.cos(long2Rad - long1Rad))
            ))
        )))
    );
}
